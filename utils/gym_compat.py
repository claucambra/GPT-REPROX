...
"""
Some widely-used versions of gym did not come with the Text space type.
We therefore selectively define a converter for the old Text type if it is
available.
"""
try:
    from gym.spaces import Text as _

    __old_gym_space_text_available = True
    ...
    __old_gym_space_text_available = False
    print("This version of gym does not have the Text space.")
...
def convert_gym_space_with_fallback_callback(old_gym_space: old_spaces.Space, 
    ...
    elif __old_gym_space_text_available and isinstance(old_gym_space, old_spaces.Text):
        return convert_gym_text(old_gym_space)
    else:
        # Don't know, so invoke the callback
